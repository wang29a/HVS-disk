...
namespace pipeann {
  ...
  void SSDIndex<T, TagT>::trigger_deletion_in_place(const tsl::robin_set<TagT> &deleted_nodes_set, uint32_t nthreads,
    ...
    for (uint64_t in_sector = 0; in_sector < n_sectors; in_sector += 128) {
      uint64_t st_sector = in_sector, ed_sector = std::min(in_sector + 128, n_sectors);
...
#pragma omp parallel for num_threads(populate_nthreads) schedule(dynamic, 1024)
