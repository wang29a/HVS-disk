...
template <class T> bool Serializer::write_block(int block_id, void *data) {
  ...
  if (1) {
    ...
    if (res <= 0) {
      printf("[DEBUG]: fd=%d offset=%zu sizeof(T)=%zu\n", fd, offset, sizeof(T));
      fprintf(stderr, "[ERROR]: pwrite64 %s, in %s:%d.\n", 
              strerror(errno),
              __FILE__,
              __LINE__);
      close(fd);
...
template <class T> int Serializer::read_block_aio(int block_id, T *block_ptr, io_context_t & ctx) {

  size_t offset = (size_t)block_id * sizeof(T);

  assert(reinterpret_cast<uintptr_t>(block_ptr) % 512 == 0);
  assert(sizeof(T) % 512 == 0);
  assert(offset % 512 == 0);
  
  struct iocb cb;
  struct iocb *cbs[1];
  memset(&cb, 0, sizeof(cb));
  io_prep_pread(&cb, fd, (char *)block_ptr, sizeof(T), offset);
  cbs[0] = &cb;
  ...
  if (bytes != sizeof(T)) {
    fprintf(stderr, "[ERROR] AIO read_block: expected %zu, got %zd bytes\n", sizeof(T), bytes);
    exit(1);
  }
...
template bool Serializer::write_block<Block>(int block_id, void *data);
template bool Serializer::write_blocks<Block>(int first_block_id,
                                                    void *data, size_t count);

template int Serializer::read_block(int block_id, Block *block_ptr);
template int Serializer::read_block_aio(int block_id, Block *block_ptr, io_context_t &ctx);
