...
log_path=$LOG_PATH/fig6
batch_num=3
mkdir -p $log_path

overall_performance=$PROJECT_PATH/tests/overall_performance.cpp
...
# sed -i "s/#define NUM_INSERT_THREADS [0-9]\+/#define NUM_INSERT_THREADS ${num_threads}/" $overall_performance
# sed -i "s/#define NUM_MERGE_THREADS [0-9]\+/#define NUM_MERGE_THREADS ${num_threads}/" $overall_performance

# sed -i "s/#define NUM_INSERT_THREADS [0-9]\+/#define NUM_INSERT_THREADS 10/" $overall_performance
# sed -i "s/#define NUM_MERGE_THREADS [0-9]\+/#define NUM_MERGE_THREADS 20/" $overall_performance
...
do
    ...
    for data_name in sift100w_shuffled #sift100w_shuffled msong text gist
    do
        L='100'
        batch_size=1000
